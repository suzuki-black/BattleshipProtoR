...
- **★連続縦スクロール地形エンジン(SC_STAGE)**: 海と戦艦バトルは**地続き(画面カット無し)の1本の縦スクロール**。
  page1を256pxリングバッファ(R#23)にし、戦艦は上位VRAM(バッファB)へ事前描画→露出行をLMMMで流し込む。
  海=蛇行なし直進→船尾が見えたら地続きに交戦→戦艦=**船首↔船尾の往復蛇行(縦往復＋横揺れ)**(=前作の戦艦戦)。
  openMSXで「海スクロール→戦艦が上から地続き登場→往復蛇行」を確認。フロー boot→title→config→**SC_STAGE**。
  (旧SC_INTRO/SC_BOSSは統合予定。砲塔/戦闘機/HP/HUDは次段でSC_STAGEへ載せる。)
  - 常駐コード 7814B / 24KB(残り16.4KB)。
