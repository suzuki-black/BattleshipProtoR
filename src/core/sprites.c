...
/* ===== 主砲の可動砲身(8方向)。★方向いっぱいに貫く太いバー(対角は角→角=切れない)。
   向きは bh_turret でスプライトをその方向へ数px オフセットして表現(ドームから砲身が突き出る)。
   上下は同一バー(縦), 左右も同一(横), 斜めも対で同一=実質4パターン。行別カラーで金属シェード。 ===== */
...
const u8 barrel_col[16]   = { 1,1,4,4,14,14,15,15,15,15,14,14,4,4,1,1 };  /* 暗いガンメタル砲身(明るいドームと対比。黒縁→紺→白ハイライト) */
