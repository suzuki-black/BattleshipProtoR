...
static void stage_intro(void) {
    ...
    vdp_fill(0, 8, 256, 2, 8);             /* 上装飾ライン(赤) */
    vdp_fill(0, 176, 256, 2, 8);           /* 下装飾ライン(赤) */

    num[0] = (char)('1' + curstage); num[1] = 0;
    vdp_text(92, 16, 14, 1, "STAGE");
    vdp_text(140, 16, 15, 1, num);
    vdp_text(72, 32, 8, 1, "- TARGET WARSHIP -");

    /* 額縁ウィンドウ(枠=白/内側=海)。中の艦画像は stage_build 後に実艦BGから切り出してコピーする。 */
    vdp_fill(92, 46, 72, 108, 15);         /* 枠(白) 外周 */
    vdp_fill(95, 49, 66, 102, 4);          /* 内側=海(青)(コピーで上書きされる下地) */

    vdp_text((u8)(128 - n * 4), 160, 15, 1, nm);   /* 艦名(中央寄せ) */

    stage_build();                          /* ★カードの裏でステージ準備。艦はバッファB(実描画)へ */

    /* ★実艦BG(バッファB=SC_SHIPBUF_Y)の胴体中央部(上構〜煙突, world-y 120..220)を額縁内へ
       close-up コピー(LMMM)。run_ops の幾何再描画ではなく「ゲームに実際に出る艦そのもの」を額装
       =本物のプレビュー。艦は buffer B の x100..156 に居るので x96始点(海の余白込み64px)で切り出す。 */
    vdp_copy(96, (u16)(SC_SHIPBUF_Y + 120), 96, 50, 64, 100);
